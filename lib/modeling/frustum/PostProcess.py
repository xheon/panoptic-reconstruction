...
class PostProcess(nn.Module):
    ...
    def forward(self, instance_data: Dict[str, torch.Tensor], frustum_data: Dict[str, torch.Tensor]) -> ModuleResult:
        ...
        instances_filtered = self.filter_instances(instance_data, frustum_data)

        # merge output
        panoptic_instances = torch.zeros_like(frustum_data["geometry"])
        panoptic_semantics = {}

        things_start_index = 2  # map wall and floor to id 1 and 2

        surface_mask = frustum_data["geometry"].abs() < 1.0

        # Merge things classes
        for index, instance_id in enumerate(instances_filtered):
            # Ignore freespace
            if instance_id != 0:
                # Compute 3d instance surface mask
                instance_mask: torch.Tensor = torch.Tensor(frustum_data["instance"] == instance_id)
                ...
                semantic_region = torch.masked_select(frustum_data["semantic"], instance_surface_mask)
        ...
        wall_class = 10
        wall_id = 1
        wall_surface_mask = frustum_data["semantic"] == wall_class & surface_mask
        ...
        floor_class = 11
        floor_id = 2
        floor_surface_mask = frustum_data["semantic"] == floor_class & surface_mask
