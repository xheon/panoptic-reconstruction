# Adopted from https://github.com/facebookresearch/ContrastiveSceneContexts/tree/main/downstream/insseg/lib
...
class BallQueryBatchP(Function):
    ...
    def forward(ctx, coords, batch_idxs, batch_offsets, radius, meanActive):
        ...
        while True:
            # idx = torch.IntTensor(n * meanActive).zero_()
            idx = torch.cuda.IntTensor(n * meanActive).zero_()
            # start_len = torch.IntTensor(n, 2).zero_()
...
class Clustering:
    def __init__(self, ignored_labels, class_mapping, thresh=0.03,
        ...
        self.class_mapping = class_mapping.to(config.MODEL.DEVICE)
        self.propose_points = propose_points
        self.score_func = score_func

    def cluster(self, vertices, scores):
        labels = torch.max(scores, 1)[1]  # (N) long, cuda
        proposals_idx, proposals_offset = self.cluster_(vertices, labels.to(config.MODEL.DEVICE))
    ...
    def cluster_(self, vertices, labels):
        ...
        batch_idxs_ = batch_idxs[object_idxs].int().to(config.MODEL.DEVICE)
        batch_offsets_ = torch.FloatTensor([0, object_idxs.shape[0]]).int().to(config.MODEL.DEVICE)
